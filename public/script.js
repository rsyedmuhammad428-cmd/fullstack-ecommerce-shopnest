...
let wishlist = JSON.parse(localStorage.getItem('cWish')) || [];
let curProd = null;
let pickedStar = 0;
let toastTmr;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Restore SPA state from cross-page navigation
  const hash = window.location.hash.substring(1);
  if (hash.startsWith('home-')) {
      const section = hash.substring(5);
      nav('home');
      setTimeout(() => navToSection(section), 350);
  } else if (hash.startsWith('shopCat-')) {
      const cat = decodeURIComponent(hash.substring(8));
      navToShopCat(cat);
  } else if (hash.startsWith('search-')) {
      const query = decodeURIComponent(hash.substring(7));
      document.getElementById('srchInp').value = query;
      doSearch();
  } else if (hash && document.getElementById('pg-' + hash)) {
      _showPage(hash, true);
  }

  // Global Header Navigation Locks
  document.querySelectorAll('.hbtn[title="Account"]').forEach(btn => {
      btn.onclick = () => {
          const token = localStorage.getItem('shopnest_token');
          window.location.href = token ? 'account.html' : 'login.html';
      };
  });
  document.querySelectorAll('.hbtn[title="Wishlist"]').forEach(btn => {
      btn.onclick = () => window.location.href = 'wishlist.html';
  });
});
...
function doSearch() {
  ...
  if (!document.getElementById('pg-shop')) {
...
function navToSection(sectionId, e) {
  ...
  if (!document.getElementById('pg-home')) {
      window.location.href = 'index.html#home-' + sectionId;
      return;
  }
...
function navToShopCat(cat, e) {
  ...
  if (!document.getElementById('pg-shop')) {
      window.location.href = 'index.html#shopCat-' + encodeURIComponent(cat);
      return;
  }
...
function card(p) {
  ...
  <div class="pcard" onclick="openDetail(${p.id})">
    <div class="pi-wrap">
      ...
      <button class="p-wish" onclick="togWish(this,${p.id},event)" title="Save to Wishlist"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z"/></svg></button>
...
function renderCart() {
  localStorage.setItem('cCart', JSON.stringify(cart));
...
function togWl(btn) {
  ...
  if (on) {
    if (!wishlist.includes(curProd.id)) wishlist.push(curProd.id);
  } else {
    wishlist = wishlist.filter(id => id !== curProd.id);
  }
  ...
  if (window.toggleWishlistAPI) window.toggleWishlistAPI(curProd.id);
}

function togWish(btn, id, e) {
  e.stopPropagation();
  ...
  if (window.toggleWishlistAPI) window.toggleWishlistAPI(id);
  showToast(on ? 'Saved to wishlist ♥' : 'Removed from wishlist');
}
...
function showToast(msg) {
  ...
  if (!t || !tMsg) {
    ...
    t.className = 'toast';
    t.id = 'toast';
    tMsg = document.createElement('div');
    tMsg.id = 'toastMsg';
    t.appendChild(tMsg);
